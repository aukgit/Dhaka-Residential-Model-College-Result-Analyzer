...
Public Class AukFunC2
    ...
    Public OdbcAdp As Data.Odbc.OdbcDataAdapter
    'Public SqlAdp As Data.SqlClient.SqlDataAdapter
    Public OdbcConnect As New Odbc.OdbcConnection(Connect)
    Dim CstrType As String
    Public Function Check_Data_Object_Is_ChageOrNot(ByVal Obj As Object, Optional ByVal ShowSaveMsg As Boolean = False) As DialogResult
        Dim T As DataTable = Me.Get_DataSystemObj(Obj)
        Try
            rqa = T.GetChanges.Rows.Count
        Catch ex As Exception
            rqa = 0
        End Try
        If rqa = 0 Then
            Check_Data_Object_Is_ChageOrNot = DialogResult.Yes
            ChkChg = True
        Else
            Check_Data_Object_Is_ChageOrNot = DialogResult.No
            ChkChg = False
        End If
        If ShowSaveMsg = True Then
            If Check_Data_Object_Is_ChageOrNot = True Then
                Check_Data_Object_Is_ChageOrNot = MsgBox(What & "Save (" & T.TableName & ") this DataTable's ChangesData?", MsgBoxStyle.YesNoCancel, "Save")
    ...
    Public Function BindFilter_BetweenOparetor(ByVal Bp As BindingSource, ByVal D_Field As String, ByVal FindTxt As String, Optional ByVal NotOparetor As Boolean = False, Optional ByVal WithFilter As Boolean = False, Optional ByVal SetInDatatable As Boolean = False, Optional ByVal ClrBefore As Boolean = True, Optional ByVal BindingFilter As Boolean = True, Optional ByVal OrdFields As String = "", Optional ByVal OpenTitles As String = "*") As String
        ...
        Me.CutWordLetter(Lst, FindTxt, ",")
        If IsNumeric(D_Field) Then
            D_Field = T.Columns.Item(D_Field).ColumnName.ToString
        End If
        Dim BStr, Str, nStr As String
        If WithFilter = True Then
            Try
                BStr = Bp.Filter.ToUpper
            Catch ex As Exception
                BStr = ""
            End Try
        Else
            BStr = ""
        End If
        If NotOparetor = True Then
            nStr = " Not "
        Else
            nStr = ""
        End If
        If Me.Chk_DataTable_Field_NumericOrNot(Bp, D_Field) = False Then
            Str = "([" & D_Field & "]" & nStr & " Between '" & ZA(0) & "' AND '" & ZA(1) & "')"
        Else
            If CstrType = "System.DateTime" Then
                Str = "([" & D_Field & "]" & nStr & " Between #" & ZA(0) & "# AND #" & ZA(1) & "#)"
            Else
                Str = "([" & D_Field & "]" & nStr & " Between " & ZA(0) & " AND " & ZA(1) & ")"
            End If
        End If
        If BindingFilter = True Then
            If BStr.Trim <> "" Then
                Bp.Filter = "(" & BStr.ToUpper & ") and " & Str
            Else
                Bp.Filter = Str
            End If
            If OpenTitles = "*" Then
                OrdTableName = OrdFields
                Me.DataTableFilter(Bp, ClrBefore, False, True, Me.BoolInvert(BindingFilter), -1)
            Else
                OrdTableName = OrdFields
                Me.DataTableFilter(Bp, ClrBefore, False, False, Me.BoolInvert(BindingFilter), -1)
            End If
            BindFilter_BetweenOparetor = Bp.Filter
            'Exit Function
        End If
        If SetInDatatable = True Then
            If BStr.Trim <> "" Then
                Str = "(" & BStr.ToUpper & ") and " & Str
            Else
                Str = Str
            End If
            'Bp.Filter = Str
            'SFC("a")
            ExpText(Str)
            Me.Db_Load(OpenTitles, T, T.TableName, ClrBefore, OrdFields, False, False, -1, 0)
            'MsgBox(Sql)
            'BindFilter_BetweenOparetor = Sql
        End If
    End Function
    Public Function Mother_DB_Load(ByVal DbObject_To_Load As Object, ByVal GetInColumn As Integer)
        ...
        For Aq = 0 To T.Rows.Count - 1
            ...
            If Me.Chk_DataTable_Field_NumericOrNot(T, GetInColumn) = False Then
                Str = "'" & Str & "'"
            End If
            If RStv = "" Then
                RStv = Str
        ...
        Mother_DB_Load = RStv
    End Function
    Public Function Mother_DB_Load(ByVal Lst_or_Txt As Object, Optional ByVal Key As String = ",", Optional ByVal Db As Object = Nothing, Optional ByVal ForWhichColumnThisTextCollecting As Integer = -1, Optional ByVal CheckNumeric As Boolean = False, Optional ByVal SingleTxt As Boolean = True, Optional ByVal TrimSys As Boolean = True, Optional ByVal UpperCase As Boolean = False)
        Dim Str, RStv As String
        Dim Aq, SSvg As Integer
        Dim Gstr As String
        Dim Lst As New ListBox
        SSvg = ForWhichColumnThisTextCollecting
        Dim Fqa As Char
        Dim T As DataTable
        If Db IsNot Nothing Then
            T = Me.Get_DataSystemObj(Db)
        End If
        'Dim T As DataTable = Me.Get_DataSystemObj(DbObject_To_Load)
        If TypeOf (Lst_or_Txt) Is ListBox Then
            'For Aq = 0 To Lst_or_Txt.items.count - 1
            '    If Gstr = "" Then Gstr = Lst_or_Txt.items.item(Aq).ToString Else Gstr = Gstr & Key & Lst_or_Txt.items.item(Aq).ToString
            'Next
            Lst = Lst_or_Txt
        ElseIf TypeOf (Lst_or_Txt) Is TextBox Then
            Gstr = Lst_or_Txt.text
            Me.CutWordLetter(Lst, Gstr, Key, True, False, CheckNumeric, False, TrimSys, UpperCase, "", "")
        ElseIf TypeOf (Lst_or_Txt) Is String Then
            Gstr = Lst_or_Txt
            Me.CutWordLetter(Lst, Gstr, Key, True, False, CheckNumeric, False, TrimSys, UpperCase, "", "")
        Else
            MsgBox("Auk Type Not match(Str,lst,textbox).", MsgBoxStyle.Critical)
            Exit Function
        End If
        RStv = ""
        If SSvg > -1 Then
            If Me.Chk_DataTable_Field_NumericOrNot(T, SSvg) = True Then
                If CstrType = "System.DateTime" Then
                    Fqa = "#"
                Else
                    Fqa = ""
            ...
            End If
        Else
            MsgBox("Please Select Column Intejer")
            Exit Function
        ...
        For Aq = 0 To Lst.Items.Count - 1
            ...
            Str = Lst.Items.Item(Aq).ToString
    ...
    Public Function Chk_DataTable_Field_NumericOrNot(ByVal Bp As Object, ByVal D_Fld As String) As Boolean
        Dim T As DataTable = Me.Get_DataSystemObj(Bp)
        Dim TStr As String
        If IsNumeric(D_Fld) = True Then
            TStr = T.Columns.Item(CInt(D_Fld)).DataType.ToString()
        Else
            TStr = T.Columns.Item(CStr(D_Fld)).DataType.ToString()
        End If
        If TStr = "System.Decimal" Or TStr = "System.Double" Or TStr = "System.Single" Or TStr = "System.DateTime" Or TStr = "System.Double" Or TStr = "System.Int32" Or TStr = "System.Single" Or TStr = "System.Int64" Or TStr = "System.Int16" Then
            Chk_DataTable_Field_NumericOrNot = True
        Else
            Chk_DataTable_Field_NumericOrNot = False
        End If
        CstrType = TStr
    End Function
    Public Function CommonRowsAdd(ByVal Bp As Object, ByVal Row As DataRow, ByVal MastersInput As String, ByVal MasterColumn As Integer, Optional ByVal WhichColumn As String = "", Optional ByVal Changes As String = "", Optional ByVal Key As String = ",")
        ...
        If MastersInput.Trim <> "" Then
            Me.CutWordLetter(Lst, MastersInput, Key)
        End If
        If WhichColumn.Trim <> "" Then
            Me.CutWordLetter(Lst2, WhichColumn, Key)
        End If
        If Changes.Trim <> "" Then
            Me.CutWordLetter(Lst3, Changes, Key)
        End If
        For Aq = 0 To Lst.Items.Count - 1
            Str = Lst.Items.Item(Aq).ToString
            ...
            For Aq2 = 0 To CRows.Table.Columns.Count - 1
                Try
                    CL = Lst2.FindStringExact(Aq2)
                    If CL > -1 Then

                        Str2 = Lst2.Items.Item(CL).ToString
                        Str3 = Lst3.Items.Item(CL).ToString
                        If IsNumeric(Str2) = True And InStr(Str2, ",") Then
                            CRows.Item(CInt(Str2)) = Str3
                        Else
                            CRows.Item(CStr(Str2)) = Str3
                        End If
                    Else
        ...
        Next
    End Function
    Public Function Get_DataSystemObj(ByVal Obj As Object) As DataTable
        If TypeOf (Obj) Is DataTable Then
            Get_DataSystemObj = Obj
        ElseIf TypeOf (Obj) Is BindingSource Then
            Get_DataSystemObj = Me.GetTableFromBindingSouce(Obj)
            'ElseIf TypeOf (Obj) Is DataSet Then
            'Get_DataSystemObj = Me.GetTableFromBindingSouce(Obj)

        End If
    End Function
    Public Function Auk_ListObject_DataTable_DataType(ByVal Lst As Object, ByVal Obj As Object, Optional ByVal Separete As String = "", Optional ByVal SelectIndex As Integer = 0)


        Dim T As DataTable
        Dim Dig As Integer
        Dim Lstq As New ListBox

        If TypeOf (Obj) Is DataTable Then
            T = Obj
        ElseIf TypeOf (Obj) Is BindingSource Then
            T = Me.GetTableFromBindingSouce(Obj)
        Else
            MsgBox("Auk Error .. on Auk Command", MsgBoxStyle.Critical)
        End If
        Lst.Items.Clear()
        If Separete <> "" Then
            'If InStr(Separete, ",") > 0 Then
            Me.CutWordLetter(Lstq, Separete, ",")
            'End If
        End If
        For Dig = 0 To T.Columns.Count - 1

            If Lstq.FindStringExact(Dig) = -1 Then
                Lst.Items.Add(T.Columns.Item(Dig).ColumnName.ToString)
            End If
        Next
    ...
    End Function
    Public Function Odbc_SqlStatements(ByVal TitleNames As String, ByVal TableName As String, ByVal DbT As Object, Optional ByVal ClrDb As Boolean = True, Optional ByVal OrderTableTitles As String = "", Optional ByVal Like_Option As Boolean = False, Optional ByVal Like_From_First As Boolean = False, Optional ByVal Limit As Integer = -1, Optional ByVal LoadStart As Integer = 0)
        'If TypeOf DbT Is DataTable Then
        '    Dim TData As DataTable = DbT
        'ElseIf TypeOf DbT Is DataSet Then
        '    Dim TData As DataSet = DbT
        'End If
        If Like_Option = True Then
            If Like_From_First = True Then
                FirstSqlAukCmDND(TitleNames, OrderTableTitles, False, True, TableName, True)
            Else
                AukSql.SqlAukCmDND(TitleNames, OrderTableTitles, False, True, TableName, True)
            End If
        Else
            AukSql.SqlAukCmDND(TitleNames, OrderTableTitles, False, True, TableName, False)
        End If
        If ClrDb = True Then DbT.Clear()
        Try
            OdbcAdp = New Odbc.OdbcDataAdapter(Sql, OdbcConnect)
            If TypeOf DbT Is DataSet Then
                If Limit > -1 Then
                    OdbcAdp.Fill(DbT, LoadStart, Limit, TableName)
                Else
                    OdbcAdp.Fill(DbT, TableName)
                End If
            ElseIf TypeOf DbT Is DataTable Then
                If Limit > -1 Then
                    OdbcAdp.Fill(LoadStart, Limit, DbT)
                Else
                    OdbcAdp.Fill(DbT)
                End If
            Else
        ...
        End Try
    End Function
    Public Function LoadWhole_Str_Db_Set_Or_Table(ByVal bp As BindingSource, ByVal Str As String, ByVal Column As String, Optional ByVal key As String = ",", Optional ByVal Unique As Boolean = False)
        Dim T As DataTable = Me.GetTableFromBindingSouce(bp)
        Dim Clmn As String
        Dim Lst As New ListBox
        Me.CutWordLetter(Lst, Str, key, Unique, Me.BoolInvert(Unique), False)
        ...
        End If
        If T.Columns(Clmn).DataType.ToString = "System.Decimal" Or T.Columns(Clmn).DataType.ToString = "System.Double" Or T.Columns(Clmn).DataType.ToString = "System.Single" Or T.Columns(Clmn).DataType.ToString = "System.DateTime" Or T.Columns(Clmn).DataType.ToString = "System.Double" Or T.Columns(Clmn).DataType.ToString = "System.Int32" Or T.Columns(Clmn).DataType.ToString = "System.Single" Or T.Columns(Clmn).DataType.ToString = "System.Int64" Or T.Columns(Clmn).DataType.ToString = "System.Int16" Then
            If T.Columns(Clmn).DataType.ToString = "System.DateTime" Then
                MsgBox("not Set now")
                'Me.BindFilterNumber(bp, TabName, "#" & FindText & "#", Sign, withFilter)
            Else
    ...
    Public Function List_or_Combo_AddDb_Row_dataSource(ByVal xlst As Object, ByVal Bp As BindingSource, ByVal Column As String, ByVal Row As String)
        'Dim = List_or_Combo_AddDb_Row_dataSource
        Dim Lst As New ListBox
        Dim Uq As Integer
        Dim T As DataTable = Me.GetTableFromBindingSouce(Bp)
        ...
        If Res1 = False Then
            Me.CutWordLetter(Lst, Column, ",")
        End If
        If Lst.Items.Count > 0 Then
            For Uq = 0 To Lst.Items.Count - 1
                Tp = Lst.Items.Item(Uq).ToString
        ...
        End If
    End Function
    Public Function List_or_Combo_AddDb_Row_dataSource(ByVal xlst As Object, ByVal Datatable As DataTable, ByVal Column As String, ByVal Row As String)
        'Dim xlst As Object = List_or_Combo_AddDb_Row_dataSource
        Dim Lst As New ListBox
        Dim Uq As Integer
        Dim T As DataTable = Datatable
        Dim Strp, Tp As String
        Try
            xlst.Items.Clear()
    ...
    Public Function Db_Load(ByVal TitleNames As String, ByVal DbT As Object, ByVal TableName As String, Optional ByVal ClrDb As Boolean = True, Optional ByVal OrderTableTitles As String = "", Optional ByVal Like_Option As Boolean = False, Optional ByVal Like_From_First As Boolean = False, Optional ByVal Limit As Integer = -1, Optional ByVal LoadStart As Integer = 0, Optional ByVal GroupBy As Boolean = False, Optional ByVal MSG As Boolean = True)
        If ConPro = "oledb" Then
            OleDb_SqlStatements(TitleNames, DbT, TableName, ClrDb, OrderTableTitles, Like_Option, Like_From_First, Limit, LoadStart, GroupBy, MSG)
    ...
    Public Function OleDb_SqlStatements(ByVal TitleNames As String, ByVal DbT As Object, Optional ByVal TableName As String = "", Optional ByVal ClrDb As Boolean = True, Optional ByVal OrderTableTitles As String = "", Optional ByVal Like_Option As Boolean = False, Optional ByVal Like_From_First As Boolean = False, Optional ByVal Limit As Integer = -1, Optional ByVal LoadStart As Integer = 0, Optional ByVal GroupBy As Boolean = False, Optional ByVal Msg As Boolean = False)
        ...
        '    Dim TData As DataSet = DbT
        'End If
        If (TypeOf DbT Is BindingSource) Or (TypeOf DbT Is DataTable) Then
            DbT = Me.Get_DataSystemObj(DbT)
            If TableName = "" Then
                TableName = DbT.TableName.ToString
            End If
        ElseIf (TypeOf DbT Is DataSet) Then
            If TableName = "" Then
                'TableName = DbT.tablename.ToString
                MsgBox("you Must type you table name when Select DataSet")
                Exit Function
            End If
        Else
            MsgBox("Not match Type OleDb Exp.Auk.")
            Exit Function
        End If
        If TableName = "" Then
            MsgBox(TableName & " is null Please Check you Information", MsgBoxStyle.Critical)
            Exit Function
        End If
        'if 
        If Like_Option = True Then
            If Like_From_First = True Then
                AukSql.SqlAukCmDND(TitleNames, OrderTableTitles, GroupBy, Me.BoolInvert(GroupBy), TableName, True, True)
            Else
                AukSql.SqlAukCmDND(TitleNames, OrderTableTitles, GroupBy, Me.BoolInvert(GroupBy), TableName, True)
            End If
        Else
            AukSql.SqlAukCmDND(TitleNames, OrderTableTitles, GroupBy, Me.BoolInvert(GroupBy), TableName, False)
        End If
        'MsgBox(Sql)
        ...
        Try
            ...
            If TypeOf DbT Is DataSet Then
                If ClrDb = True Then
                    DbT.tables(TableName).Clear()
                End If
                If Limit > -1 Then
                    Adp.Fill(DbT, LoadStart, Limit, TableName)
                Else
                    Adp.Fill(DbT, TableName)
                End If
            ElseIf TypeOf DbT Is DataTable Then
                If ClrDb = True Then
                    DbT.Clear()
            ...
            If AllowsFormatAuk.Trim <> "" Then
                ...
                Else
                    If Msg = True Then
                        ...
                        Else
                            MsgBox("This is not Expressed with Meunction", MsgBoxStyle.Critical)
    ...
    Public Function ReplaceInBindingSource(ByVal Bp As BindingSource, ByVal EnterTxt As String, ByVal Column As String, Optional ByVal ChkNumeric As Boolean = False, Optional ByVal Key As String = vbCrLf, Optional ByVal AllowsFormatAuk As String = "", Optional ByVal CanEdit As Boolean = False, Optional ByVal Prog As Object = Nothing, Optional ByVal PrgVisibleOption As Boolean = False, Optional ByVal Msg As Boolean = False, Optional ByVal MsgTxt As String = "", Optional ByVal InsertNonNumeric As Boolean = False, Optional ByVal TrimSystemAdd As Boolean = False, Optional ByVal UpperCaseAddSys As Boolean = False, Optional ByVal AddWithSomething As String = "", Optional ByVal ReplaceFind As String = "", Optional ByVal ReplaceTxt As String = "", Optional ByVal ChangePosition As Boolean = False, Optional ByVal ReplaceCaseSenseTive As Boolean = True, Optional ByVal FindSomeThing_For_Event As String = "", Optional ByVal Call_Auk_Crv1_Even As Boolean = False, Optional ByVal InWhichForm As Object = Nothing, Optional ByVal CallEventAtFirst As Boolean = False)
        ...
        Dim Lst, Alp As New ListBox
        Dim T As DataTable = Me.GetTableFromBindingSouce(Bp)
        Me.CutWordLetter(Lst, EnterTxt, Key, False, True, ChkNumeric, InsertNonNumeric, TrimSystemAdd, UpperCaseAddSys, AddWithSomething, ReplaceFind, ReplaceTxt, ReplaceCaseSenseTive, FindSomeThing_For_Event, Call_Auk_Crv1_Even, InWhichForm, CallEventAtFirst)
        'MsgBox(Lst.Items.Count)
        If Prog IsNot Nothing Then
            If PrgVisibleOption = True Then Prog.Visible = True
            Prog.Maximum = Bp.Count
    ...
    End Function
    Public Function ReplaceInGrid2(ByVal D As DataGridView, ByVal EnterTxt As String, ByVal Column As Integer, Optional ByVal ChkNumeric As Boolean = False, Optional ByVal Key As String = Chr(13), Optional ByVal AllowsFormatAuk As String = "", Optional ByVal CanEdit As Boolean = False, Optional ByVal Prog As Object = Nothing, Optional ByVal PrgVisibleOption As Boolean = False, Optional ByVal Msg As Boolean = False, Optional ByVal MsgTxt As String = "", Optional ByVal InsertNonNumeric As Boolean = False, Optional ByVal TrimSystemAdd As Boolean = False, Optional ByVal UpperCaseAddSys As Boolean = False, Optional ByVal AddWithSomething As String = "", Optional ByVal ReplaceFind As String = "", Optional ByVal ReplaceTxt As String = "")
        ...
        Dim Aq, Aq2, MnpU As Integer
        Dim Lst, Alp As New ListBox
        Me.CutWordLetter(Lst, EnterTxt, Key, False, True, ChkNumeric, InsertNonNumeric, TrimSystemAdd, UpperCaseAddSys, AddWithSomething, ReplaceFind, ReplaceTxt, True)
        MsgBox(Lst.Items.Count)
        If Prog IsNot Nothing Then
            If PrgVisibleOption = True Then Prog.Visible = True
        ...
        If CanEdit = True Then
            If AllowsFormatAuk.Trim <> "" Then
                Me.CutWordLetter(Alp, AllowsFormatAuk.Trim, ",")
            End If
            If Alp.FindStringExact(Column) = -1 Then
                W = True
            Else
        ...
        End If
        If W = True Then
            For Aq = 0 To D.SelectedCells.Count - 1
                If Me.GetIndexInOrNot(Lst, MnpU) = True Then D.SelectedCells.Item(Aq).Value = Lst.Items.Item(MnpU).ToString
                MnpU += 1
            Next
            Try
                If Prog IsNot Nothing Then
    ...
    End Function
    Public Function ComboToAddTableNames(ByVal Bp As BindingSource, ByVal Com As Object, ByVal SperateColumnsMeormat As String, ByVal Key As String)
        Dim Le As New ListBox
        Dim d As DataSet = Bp.DataSource
        Dim t As String = Bp.DataMember.ToString
        Me.CutWordLetter(Le, SperateColumnsMeormat, Key, True, False)
    ...
    Public Function CutTxtGet(ByVal Txt As String, ByVal Key As String, ByVal NumArryGet As String) As String
        Dim Lst As New ListBox
        Me.CutWordLetter(Lst, Txt, Key, True)
    ...
    'End Function
    Public Function DelItemsFromList(ByVal Lst As Object, ByVal Txt As String, Optional ByVal Key As String = ",")
        Dim IxoQ As Integer
        Dim Lplst As New ListBox
        Me.CutWordLetter(Lplst, Txt, Key, True, True)
        For IxoQ = 0 To Lplst.Items.Count - 1
            If Me.FindInObjectAndSelect(Lst, Lplst.Items.Item(IxoQ), True, False, False) = True Then
                Lst.Items.Remove(Lplst.Items.Item(IxoQ))
            End If
        Next
    ...
    End Function
    Public Function BindFilterNot(ByVal Bp As BindingSource, ByVal TitleName As String, ByVal FindTxt As String, Optional ByVal LikeOpt As Integer = 0) As String
        If LikeOpt = 1 Then
            Bp.Filter = "[" & TitleName & "] Not Like  '%" & FindTxt & "%'"
            Bp.Filter = "(" & Bp.Filter.ToUpper & ")"
            BindFilterNot = Bp.Filter.ToUpper
        ElseIf LikeOpt = 2 Then
            Bp.Filter = "[" & TitleName & "] Not Like  '" & FindTxt & "%'"
            Bp.Filter = "(" & Bp.Filter.ToUpper & ")"
            BindFilterNot = Bp.Filter.ToUpper
        ElseIf LikeOpt = 3 Then
            Bp.Filter = "[" & TitleName & "] Not Like  '%" & FindTxt & "'"
            Bp.Filter = "(" & Bp.Filter.ToUpper & ")"
            BindFilterNot = Bp.Filter.ToUpper
        Else
    ...
    End Function
    Public Function BindFilterNot(ByVal Bp As BindingSource, ByVal TitleName As String, ByVal FindTxt As String, ByVal WithFilter As Boolean, Optional ByVal LikeOpt As Integer = 0) As String
        Dim Str As String
        If LikeOpt = 1 Then
            Str = "[" & TitleName & "] Not Like '%" & FindTxt & "%'"
            Str = "(" & Str.ToUpper & ")"
        ElseIf LikeOpt = 2 Then
            Str = "[" & TitleName & "] Not Like  '" & FindTxt & "%'"
            Str = "(" & Str & ")"
        ElseIf LikeOpt = 3 Then
            Str = "[" & TitleName & "] Not Liket '%" & FindTxt & "'"
            Str = "(" & Str & ")"
        Else
            Str = "[" & TitleName & "]<>'" & FindTxt & "'"
            Str = "(" & Str & ")"
        End If
        'MsgBox(Str)
        ...
        If WithFilter = True Then
            If Bp.Filter.ToString <> "" Then
                Bp.Filter = Bp.Filter.ToString & " and " & Str
            Else
                Bp.Filter = Str
            End If
...
bindger:
    ...
    Public Function DelRecAll(ByVal TitleOFTable As String, ByVal mdw As BindingSource) As Boolean
        ...
        End If
    End Function
    Public Function GetTableName(ByVal Bp As BindingSource) As String

        If TypeOf Bp.DataSource Is DataTable Then
            GetTableName = Bp.DataSource.TableName
        ElseIf TypeOf Bp.DataSource Is DataSet Then
            GetTableName = Bp.DataMember.ToString
        End If
    End Function
    ...
    End Function
    Public Function EmptyRowCoulms(ByVal Bp As BindingSource, ByVal Meormat2WaysColumnColumn As String, ByVal RowNumberFormat As String, Optional ByVal Key As String = ",", Optional ByVal Msg As Boolean = False, Optional ByVal AllRow As Boolean = False, Optional ByVal AllColumn As Boolean = False)
        Dim Aqt, SubInt, EInt, CyI As Integer
        If Msg = True Then
            If Me.MsgTr(What & "Set all Empty?") = False Then
                Exit Function
            End If
        End If
        Dim Lts, Rnw As New ListBox
        If AllColumn = False Then
            If Me.FindTxt(Meormat2WaysColumn, "-") = True Then
                'Me.CutWordLetter(Lts, Meormat2WaysColumn, "-")
                Me.DivideIn_To_2(Meormat2WaysColumn, "-")
                Try
                    'MsgBox(QA1)
                    'MsgBox(QA2)
                    Lts.Items.Clear()
                    For Aqt = QA1 To Val(QA2) - 1
                        Lts.Items.Add(Aqt)
                    Next
                Catch ex As Exception
                    Epx()
                ...
                End Try
            Else
                Me.CutWordLetter(Lts, Meormat2WaysColumn, Key)
            End If
        End If
        If AllRow = False Then
            If Me.FindTxt(RowNumberFormat, "-") = True Then
                'Me.CutWordLetter(Lts, Meormat2WaysColumn, "-")
                Me.DivideIn_To_2(RowNumberFormat, "-")
                Try
                    'MsgBox(QA1)
                    'MsgBox(QA2)
                    Rnw.Items.Clear()
                ...
                End Try
            Else
                Me.CutWordLetter(Rnw, RowNumberFormat, Key)
            End If
        End If
        If AllRow = False Then
            Dim T As DataTable = Me.GetTableFromBindingSouce(Bp)
            For Aqt = 0 To Rnw.Items.Count - 1
                SubInt = Rnw.Items.Item(Aqt).ToString
                If AllColumn = False Then
                    For EInt = 0 To Lts.Items.Count - 1
                        CyI = Lts.Items.Item(EInt).ToString
                        T.Rows(SubInt).Item(CyI) = ""
                    Next
                Else
                    For EInt = 0 To T.Columns.Count - 1
                        'CyI = Lts.Items.Item(EInt).ToString
                        T.Rows(SubInt).Item(EInt) = ""
                    Next
                End If
            Next
        ElseIf AllRow = True Then

            Dim T As DataTable = Me.GetTableFromBindingSouce(Bp)
            For Aqt = 0 To T.Rows.Count - 1
                SubInt = Aqt
                If AllColumn = False Then
                    For EInt = 0 To Lts.Items.Count - 1
                        CyI = Lts.Items.Item(EInt).ToString
    ...
    Public Function DivideIn_To_2(ByVal Str As String, ByVal Find As String, Optional ByVal MathCh As Boolean = False, Optional ByVal A1 As Object = Nothing = Nothing, Optional ByVal A2 As String = Nothing) As String
        If MathCh = False Then
            dex = InStr(UCase(Str), UCase(Find))
        Else
            dex = InStr(Str, Find)
        End If
        If dex <> 0 Then
            If A1 IsNot Nothing Then
                A1 = Str.Substring(1, dex)
            End If
            DivideIn_To_2 = Str.Substring(0, dex - 1)
            QA1 = DivideIn_To_2
            If A2 IsNot Nothing Then
                A2 = Str.Replace(QA1 & Find, "")
...
cnn:
    ...
    Public Function TransparentFrm(ByRef frm As System.Windows.Forms.Form, ByRef MakeTrans_ As Byte) As Boolean
        ...
        SetWindowLong(frm.Handle.ToInt32, GWL_EXSTYLE, WS_EX_LAYERED)
        SetLayeredWindowAttributes(frm.Handle.ToInt32, 0, MakeTrans_, LWA_ALPHA)
...
d:
    ...
    Public Function CutWordLetter(ByVal AddingList As ListBox, ByVal txt As String, ByVal Key As String, ByVal SingleTxt As Boolean, ByVal BlankAdd As Boolean, Optional ByVal ChkNemericEntry As Boolean = False, Optional ByVal InsertNonNermeric As Boolean = False, Optional ByVal TrimSystemAdd As Boolean = False, Optional ByVal UpperCaseAddSys As Boolean = False, Optional ByVal AddWithSomething As String = "", Optional ByVal ReplaceFind As String = "", Optional ByVal ReplaceTxt As String = "", Optional ByVal ReplaceCaseSenseTive As Boolean = True, Optional ByVal FindSomeThing_For_Event As String = "", Optional ByVal Call_Auk_Crv1_Even As Boolean = False, Optional ByVal InWhichForm As Object = Nothing, Optional ByVal CallEventAtFirst As Boolean = False)
        Dim Gh, nStr As String
        Dim RepLst, RepLst2, Lnq As New ListBox
        Dim DbInt, Svg1 As Integer
        ComLst = AddingList
d:
        'txt=trim(txt)
        'MsgBox(txt)
                    ...
                    If FindSomeThing_For_Event <> "" Then
                        If Call_Auk_Crv1_Even = True Then
                            If TypeOf (InWhichForm) Is Form Then
                                Try
                                    'MsgBox(InStr(FindSomeThing_For_Event, ","))
                                    If InStr(FindSomeThing_For_Event, ",") = 0 Then
                                        If InStr(Gh, FindSomeThing_For_Event) > 0 Then
                                            'ITnt = PrVI
                                            ITnt = AddingList.Items.Count
                                            InWhichForm.Auk_Crv1()
                                        End If
                                    Else
                                        If Lnq.Items.Count = 0 Then
                                            Me.CutWordLetter(Lnq, FindSomeThing_For_Event, ",")
                                            'Me.CutWordLetter(Lnq2, FindSomeThing_For_Event, Key)
                                        End If
                                        For Svg1 = 0 To Lnq.Items.Count - 1
                                            nStr = Lnq.Items.Item(Svg1).ToString.Trim
                                            If Gh.Trim.ToString = nStr.ToString Then
                                                'ITnt = PrVI
                                                'MsgBox(EStr)
                                                EStr = nStr
                                                ITnt = AddingList.Items.Count
                    ...
                    If InStr(ReplaceFind, Key) > 0 Then
                        ...
                        If ReplaceCaseSenseTive = False Then
                            For DbInt = 0 To RepLst.Items.Count - 1
                                R1 = RepLst.Items.Item(DbInt).ToString
                                R2 = RepLst2.Items.Item(DbInt).ToString
                                Gh = Replace(Gh, R1, R2, 1, -1, CompareMethod.Text)
                            Next
                        Else
                            For DbInt = 0 To RepLst.Items.Count - 1
                                Gh = Gh.Replace(RepLst.Items.Item(DbInt).ToString, RepLst2.Items.Item(DbInt).ToString)
                            Next
                        End If
                    Else
                        If ReplaceCaseSenseTive = False Then
                            'MsgBox("Upper" & Gh.Replace(ReplaceFind, ReplaceTxt))
                            Gh = Replace(Gh, ReplaceFind, ReplaceTxt, 1, -1, CompareMethod.Text)
                        Else

                            Gh = Gh.Replace(ReplaceFind, ReplaceTxt)
                        End If
                    End If
                End If
                ...
                If CallEventAtFirst = False Then
                    If FindSomeThing_For_Event <> "" Then
                        If Call_Auk_Crv1_Even = True Then
                            If TypeOf (InWhichForm) Is Form Then
                                Try
                                    If Me.FindTxt(FindSomeThing_For_Event, ",") = False Then
                                        If InStr(Gh, FindSomeThing_For_Event) > 0 Then
                                            'ITnt = PrVI
                                            ITnt = AddingList.Items.Count
                                            InWhichForm.Auk_Crv1()
