...
Public Class AukFunC2
    ...
    Public Function ReplaceInGrid(ByVal D As DataGridView, ByVal EnterTxt As String, ByVal Column As Integer, Optional ByVal ChkNumeric As Boolean = False, Optional ByVal Key As String = Chr(13), Optional ByVal AllowsFormatAuk As String = "", Optional ByVal CanEdit As Boolean = False, Optional ByVal Prog As Object = Nothing, Optional ByVal PrgVisibleOption As Boolean = False, Optional ByVal Msg As Boolean = False, Optional ByVal MsgTxt As String = "", Optional ByVal InsertNonNumeric As Boolean = False, Optional ByVal TrimSystemAdd As Boolean = False, Optional ByVal UpperCaseAddSys As Boolean = False, Optional ByVal AddWithSomething As String = "", Optional ByVal ReplaceFind As String = "", Optional ByVal ReplaceTxt As String = "")
        ...
        Dim Aq, Aq2, MnpU As Integer
        Dim Lst, Alp As New ListBox
        Me.CutWordLetter(Lst, EnterTxt, Key, False, True, ChkNumeric, InsertNonNumeric, TrimSystemAdd, UpperCaseAddSys, AddWithSomething, ReplaceFind, ReplaceTxt.ToUpper, True)
        'MsgBox(Lst.Items.Count)
        If Prog IsNot Nothing Then
            If PrgVisibleOption = True Then Prog.Visible = True
    ...
    Public Function FindTxt(ByVal txt As String, ByVal FndTxt As String, Optional ByVal MatchCase As Boolean = False) As Boolean
        If MatchCase = True Then
            q = InStr(txt, FndTxt)
        Else
            q = InStr(UCase(txt), UCase(FndTxt))
...
bindger:
    ...
    Public Function Grid_View_SetCommonItems(ByVal D As DataGridView, ByVal InputText As String, ByVal Columns As String, Optional ByVal AllRecords As Boolean = False, Optional ByVal Prog As ProgressBar = Nothing, Optional ByVal ProgVisibleOpt As Boolean = False, Optional ByVal SetVisible_Opt As Boolean = False)
        ...
        If AllRecords = False Then
            For AQ = 0 To D.SelectedRows.Count - 1
                ...
                D.SelectedRows.Item(AQ).DataGridView(Columns, Inty).Value = InputText
                If Prog IsNot Nothing Then
                    Me.InsPro(Prog, 1)
                End If
            Next
        Else
            For AQ = 0 To D.RowCount - 1
                D.CurrentCell.DataGridView(Columns, AQ).Value = InputText
                If Prog IsNot Nothing Then
                    Me.InsPro(Prog, 1)
                End If
            Next
    ...
    Public Function DivideIn_To_2(ByVal Str As String, ByVal Find As String, Optional ByVal MathCh As Boolean = False, Optional ByVal A1 As Object = Nothing = Nothing, Optional ByVal A2 As String = Nothing) As String
        ...
        If dex <> 0 Then
            ...
            End If
            DivideIn_To_2 = Str.Substring(1, dex)
            QA1 = DivideIn_To_2
            If A2 IsNot Nothing Then
                A2 = Str.Remove(Val(dex) + Find.Length, Str.Length)
            End If
            QA2 = Str.Remove(Val(dex) + Find.Length, Str.Length)
        Else
            QA2 = ""
            QA1 = ""
            DivideIn_To_2 = ""
        ...
        For PrVI = 1 To Len(txt)
            If Mid(txt, PrVI, Len(Key)) = Key Then
                ...
                If ReplaceFind <> "" Then
                    If InStr(ReplaceFind, Key) > 0 Then
                        ...
                        If ReplaceCaseSenseTive = False Then
                            For DbInt = 0 To RepLst.Items.Count - 1
                                Gh = Gh.ToUpper.Replace(UCase(RepLst.Items.Item(DbInt)), (RepLst2.Items.Item(DbInt)))
                            Next
                        Else
                            For DbInt = 0 To RepLst.Items.Count - 1
                    ...
                    Else
                        If ReplaceCaseSenseTive = False Then
                            Gh = Gh.ToUpper.Replace((ReplaceFind), (ReplaceTxt))
                        Else
                ...
                If ReplaceFind <> "" Then
                    ...
                    Else
                        If ReplaceCaseSenseTive = False Then
                            Gh = Gh.ToUpper.Replace((ReplaceFind), (ReplaceTxt))
                        Else
                            MsgBox("gone" & Gh.Replace(ReplaceFind, ReplaceTxt))
